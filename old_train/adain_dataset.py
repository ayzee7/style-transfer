...
transform = transforms.Compose([transforms.RandomCrop(256),transforms.ToTensor(),normalize])

class PreprocessDataset(Dataset):
    def __init__(self, content_dir, style_dir, trans=transform):
    ...
    def __getitem__(self, index):
        ...
        if self.trans:
            content_image = self.trans(content_image)
            style_image = self.trans(style_image)
